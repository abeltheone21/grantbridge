    ...
    <div className="flex min-h-screen bg-[#12150F] pt-14 sm:pt-16">
        ...
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <div className="absolute right-0 top-0 bottom-0 w-80 max-w-[85vw] bg-[#1C2117] shadow-2xl overflow-y-auto">
            <div className="sticky top-0 bg-[#1C2117] z-10 p-5 border-b border-[#4E5B2A]/20 flex justify-between items-center">
              <h2 className="font-bold text-[#E7E4D8] text-lg">Filters</h2>
              <button onClick={() => setShowMobileFilters(false)} className="w-8 h-8 flex items-center justify-center rounded-full bg-[#12150F] border border-[#4E5B2A]/30 text-[#A6A99F] hover:text-[#E7E4D8]" aria-label="Close filters"><FaTimes size={14} /></button>
            </div>
            <div className="p-5">
      ...
      <main className="flex-1 p-3 sm:p-6 lg:p-10 min-w-0 overflow-hidden">
        <div className="flex items-center justify-between gap-3 mb-4 sm:mb-6">
          ...
          <button onClick={exportToCSV} className="hidden sm:flex items-center gap-2 px-3 py-2 bg-[#1C2117] border border-[#4E5B2A]/30 text-[#C6A15B] rounded-lg hover:bg-[#242A1D] transition text-xs sm:text-sm shrink-0"><FaDownload className="text-xs" /> Export CSV</button>
        </div>
        <div className="flex gap-2 mb-3 sm:mb-4">
          <div className="relative flex-1 min-w-0">
            <input type="text" placeholder="Search grants..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full px-3 py-2.5 pl-10 sm:px-4 sm:py-3 sm:pl-12 bg-[#1C2117] border border-[#4E5B2A]/30 rounded-lg sm:rounded-xl focus:outline-none focus:ring-2 focus:ring-[#C6A15B] text-[#E7E4D8] placeholder-[#6C6F66] text-sm" />
            <FaSearch className="absolute left-3 sm:left-4 top-1/2 -translate-y-1/2 text-[#6C6F66] text-xs sm:text-sm" />
          </div>
          <button onClick={() => setShowMobileFilters(true)} className="md:hidden px-3 py-2.5 bg-[#1C2117] border border-[#4E5B2A]/30 rounded-lg text-[#E7E4D8] shrink-0 flex items-center gap-1.5 text-sm"><FaFilter className="text-xs" /> Filters {hasActiveFilters && `(${allActiveFilters.length})`}</button>
        </div>
        {hasActiveFilters && (
        ...
        {loading ? (
              ...
              <div key={g.id} className="bg-[#1C2117] rounded-xl border border-[#4E5B2A]/20 hover:border-[#C6A15B]/30 hover:bg-[#242A1D] transition overflow-hidden flex flex-col group">
                ...
                <div className="p-4 sm:p-5 flex-1 flex flex-col">
                  <div className="flex justify-between items-start gap-2 mb-2">
                    ...
                    <span className={`px-2 py-0.5 text-[10px] sm:text-xs rounded-full whitespace-nowrap shrink-0 ${g.status === 'published' ? 'bg-[#3F4F24]/30 text-[#C6A15B]' : 'bg-[#4E5B2A]/10 text-[#A6A99F]'}`}>{g.status === 'published' ? 'Open' : g.status === 'forecasted' ? 'Forecasted' : 'Closed'}</span>
                  </div>
          ...
          <div className="text-center py-12"><p className="text-[#A6A99F] text-sm sm:text-lg">No grants found</p>{hasActiveFilters && <button onClick={clearAllFilters} className="mt-3 text-xs sm:text-sm text-[#C6A15B] hover:text-[#d4b46d] underline">Clear all filters</button>}</div>
        )}
      </main>
    </div>
  );
...
function SidebarButtons({ exportToCSV, showSaveDialog, setShowSaveDialog, savedSearches, showSavedSearches, setShowSavedSearches, saveSearchName, setSaveSearchName, saveSearch, savingSearch, loadSearch, deleteSearch }: any) {
  return (
    <div className="p-5 border-t border-[#4E5B2A]/20 space-y-2 shrink-0">
      ...
      {savedSearches.length > 0 && (<button onClick={() => setShowSavedSearches(!showSavedSearches)} className="w-full px-4 py-2.5 text-[#A6A99F] text-xs hover:text-[#C6A15B] transition text-center">{showSavedSearches ? 'Hide' : 'Show'} Saved ({savedSearches.length})</button>)}
      {showSaveDialog && (
        <div className="pt-3">
  ...
  return (
    <div className="border-b border-[#4E5B2A]/10 pb-2 mb-2">
      <button onClick={onToggle} aria-label={`${isOpen ? 'Collapse' : 'Expand'} ${title}`} className="flex items-center justify-between w-full text-left py-2 text-[#E7E4D8] font-semibold text-xs sm:text-sm hover:text-[#C6A15B] transition-colors">
        {title}{isOpen ? <FaChevronUp className="text-[10px] shrink-0" aria-hidden="true" /> : <FaChevronDown className="text-[10px] shrink-0" aria-hidden="true" />}
      </button>
